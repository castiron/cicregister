...
class LoginController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController
{
	...
	{
		$arguments = ['loginAttempt' => $loginAttempt, 'loginType' => $loginType];
		...
		if ($this->userIsAuthenticated) {
			if ($loginAttempt || $loginHash) {
				...
				if ($redirectUrl) {
					...
					$this->forward('logout', null, null, $arguments);
				}
			} else {
				$this->forward('logout', null, null, $arguments);
			}
		} else {
			$this->forward('login', null, null, $arguments);
		}
	}
	...
	/**
		...
		*
		*/
	protected function logoutAction($loginAttempt = false, $loginType = '')
	{
		...
		$this->view->assign('loginAttempt', $loginAttempt);
	}
